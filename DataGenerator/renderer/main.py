...
import utility.setting as setting
import utility.camera_utils as camera_utils 
import utility.cc_texture_loader as cc_texture_loader
import utility.object_utils as object_utils 
...
class ObjectSampler:
    def __init__(self, objects, ground,
        ...
        if self.adjust_to_objects:
            self.min_height = 0.1
            self.max_height = self.obj_max_length
    ...
    def sample_function(self):
        def sample_pose(objs: bproc.types.MeshObject):
            ...
            objs.set_location(bproc.sampler.upper_region(
                ...
                max_height=self.max_height*self.sampling_height_factor,
...
def load_objects(object_dir: str, class_instance_num: dict, 
    ...
    :param object_dir:
    :param class_instance_num:
    :param file_type:
    :param is_from_ShapeNet:
    :param linear_damping:
    :param friction:
    :param random_color:
    :param keep_loaded_material:
    :param define_cc_material:
    :param random_material:
    :param cc_textures_dir:
...
def add_flying_disctractor(ground: bproc.types.MeshObject,
                           ...
                           Config=None):
    """ Adding the flying distractors......

    :param ground:
    :param objects:
    :param number:
    :param defined_size:
    :param adjust_size_to_objects:
    :param min_height:
    :param max_height:
    :param sampling_region_percentage:
...
def load_grounds(ground_dir:str, file_type: str=".obj",
    ...
    for ground_path in ground_paths:
        ...
        if len(gnd)>1:
            merged_gnd = object_utils.merge_objects(gnd)
            grounds.append(merged_gnd)
        elif len(gnd)==1:
            grounds.extend(gnd)
...
def create_ground(ground_dir: str=None, objects: Optional[List[bproc.types.MeshObject]]=None, 
    ...
    :param euler_rotZ:
    :param plane_xy:
    :param adjust_plane_size_to_objects:
    :param linear_damping:
    :param keep_loaded_material:
    :param define_cc_material:
    :param random_material:
    ...
    if not grounds:
        ...
        if objects and adjust_plane_size_to_objects:
            ...
            ground = bproc.object.create_primitive('PLANE', scale=[max_length*len(objects)*3, max_length*len(objects)*3, 1])
        else:
...
def set_ceiling_light(ground: bproc.types.MeshObject,
                      ...
                      Config=None):
    """ Sets the ceiling lights..

    :param ground:
    :param enabled:
    :param plane_xy:
    :param define_position:
    :param random_position:
    :param min_height:
    :param define_intensity:
    :param random_intensity:
    :param min_intensity:
    ...
    if number>0:
        ...
        for i in range(number):
            if random_position:
                ...
                location = np.array(define_position[i])
...
def set_up_camera(image_width: int=1920, 
...
def sample_camera(objects: Optional[List[bproc.types.MeshObject]]=None,
    ...
    :param min_elevation:
    :param max_elevation:
    :param prefer_small_radius:
    :param random_rotation:
    :param check_no_world_background:
    :param check_all_objects_visible:
        ...
        for i in range(10):
            for j in range(30):
                if random_position:
                    ...
                    position = np.array(define_position)
                ...
                if camera_utils.is_background_seen(cam2world_matrix):
            ...
            if check_all_objects_visible:
                fully_visible_objs = camera_utils.fully_visible_objects(objects)
...
def COCO_annotater(rendered_data, label_map, output_dir, object_occlusion,
    ...
    :param label_map:
    :param output_dir:
    :param object_occlusion:
    :param save_annotated_image:
...
def render_with_config(obj_dir, gnd_dir, class_instance_num, output_dir, config_file):
    ...
    ##############################################################################
    setting_time = timer()
    with open(os.path.join("time_stamp.txt"), "a") as myfile:
        myfile.write("Setting time -- {}s\n".format(setting_time-start))
    ...
    load_obj_time = timer()
    with open(os.path.join("time_stamp.txt"), "a") as myfile:
        myfile.write("load obj time -- {}s\n".format(load_obj_time-setting_time))
    ...
    load_gnd_time = timer()
    with open(os.path.join("time_stamp.txt"), "a") as myfile:
        myfile.write("load gnd time -- {}s\n".format(load_gnd_time-load_obj_time))
    ...
    for i in range(3):
        ...
        obj_sample_time = timer()
        with open(os.path.join("time_stamp.txt"), "a") as myfile:
            myfile.write("obj sampling time -- {}s\n".format(obj_sample_time-load_gnd_time))

        simulator.simulate()
        simu_time = timer()
        with open(os.path.join("time_stamp.txt"), "a") as myfile:
            myfile.write("Simu time -- {}s\n".format(simu_time-obj_sample_time))
        ...
        light_time = timer()
        with open(os.path.join("time_stamp.txt"), "a") as myfile:
            myfile.write("lighting time -- {}s\n".format(light_time-simu_time))


        if sample_camera(objects=objs_to_position, Config=CONF):
            cam_time = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
                myfile.write("cam time -- {}s\n".format(cam_time-light_time))
            distractors = add_flying_disctractor(ground, objs_to_position, Config=CONF)
            distractor_time = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
                myfile.write("distractor time -- {}s\n".format(distractor_time-cam_time))
            objects=camera_utils.visible_objects(objs_to_position)
            object_occlusion = object_utils.check_occlusion(objects, bproc.camera.get_camera_pose(), [ground]+distractors)
            occlusion_time = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
                myfile.write("occlusion time -- {}s\n".format(occlusion_time-distractor_time))
            data = bproc.renderer.render()
            data.update(bproc.renderer.render_segmap(map_by=["class", "instance", "class_name"], default_values={"class_name": None}))
            render_time = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
                myfile.write("render time -- {}s\n".format(render_time-occlusion_time))
            COCO_annotater(data, label_map, output_dir, object_occlusion, Config=CONF)
            anno_time = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
                myfile.write("anno time -- {}s\n".format(anno_time-render_time))
            print("Successfull")
            end = timer()
            with open(os.path.join("time_stamp.txt"), "a") as myfile:
...
obj_dir = "/BlenderProc2/object/Ball_Socket_Head_Screw"
gnd_dir = "/BlenderProc2/ground/tisch"
output_dir = "/BlenderProc2/Dataset/Test"
config_file = "/BlenderProc2/Config/experiment2.json"
class_instance_num = {'bcr-12x50': 1, 'bcr-10x40': 1, 'bcr-10x25': 1, 'bcr-16x40': 1, 'bcr-16x60': 1, 'bcr-12x30': 1, 'bcr-8x35': 1, 'bcr-8x20': 1, 'bcf-10x25': 1, 'bcf-12x30': 1, 'bcf-8x20': 1, 'bcf-6x16': 1}
